...
contract FlashSwapContract is UniswapFlashSwapper, Ownable {
    ...
    // @param _tokenBorrow The address of the token you want to flash-borrow, use 0x0000000000000000000000000000000000000000 for ETH
    ...
    function flashSwap(address _tokenBorrow, uint256 _amount, address _tokenPay, string calldata _userData) external returns(int256){
        ...
        bytes memory ud = bytes(_userData);
        startSwap(_tokenBorrow, _amount, _tokenPay, ud);
    ...
    function withdraw(address _input) external onlyOwner{
         if (_input == address(0)) {
            address(uint160(msg.sender)).transfer(address(this).balance);
            return;
        }
        IERC20(_input).transfer(msg.sender, IERC20(_input).balanceOf(address(this)));
        return;
    }
}